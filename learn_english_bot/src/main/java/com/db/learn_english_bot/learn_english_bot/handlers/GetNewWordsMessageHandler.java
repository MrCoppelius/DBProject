...
@Component
public class GetNewWordsMessageHandler implements MessageHandler {
    @Autowired
    ...
    public SendMessage menageMessage(Message message) {
        ...
        DataModel response = restTemplate.postForObject(routerServiceURL+"/get_new_words", request, DataModel.class);
        return new SendMessage(response.getChatID(), response.getWords().toString());
    }

    @Override
    public BotStatus getServiceName() {
        return BotStatus.GET_NEW_WORDS;
    }
}
